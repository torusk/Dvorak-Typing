...
function loadFromFile(file){
  if(!file){
    ...
    showStatusMessage('Please select a .txt file.');
    setFileStatus('None');
    return;
  }
  currentFileName = file.name;
  setFileStatus(`${file.name} Loading...`);
  ...
  reader.onload = ()=>{
    try{
      ...
      console.error(err);
      showStatusMessage('Failed to load.');
...
function loadDefaultText(){
  ...
  setFileStatus(`${DEFAULT_TEXT_PATH} Loading...`);
  showStatusMessage('Loading...');
...
function applyDataset(blocks){
  ...
  totalRequiredChars = 0;
  if(!exercises.length){
    showStatusMessage('No text content found.');
    return;
  }
  pickSentence();
}
...
function renderText(){
  ...
  if(!parts.length){
    parts.push(renderLine('All tasks completed.', 'status'));
  }
...
function pickSentence(){
  const set = currentSet();
  if(!set.length){
    showStatusMessage('Please select a text file.');
    return;
  }
  prepareSource();
  if(!sourceWords.length){
    showStatusMessage('No sentences found.');
    return;
  }
  wordsOffset = 0;
  cursor = 0;
